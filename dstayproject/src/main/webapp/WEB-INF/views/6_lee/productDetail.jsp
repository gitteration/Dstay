        ...
        <div class="col-sm-6 col-md-4">
            ...
            <div class="caption box">
  			...
  			<!-- 장바구니 모달창 start -->
			   ...
			   <div class="modal modal-center fade" id="testModal" tabindex="-1" role="dialog" style="top:53%; position: absolute;" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
================================================== -->
			...
			<script>
				...
				if(mno != null){	
					$.ajax({
							...
							data:{pno:pno},
							dataTpye:"json",
							success:function(data){ // ajax 성공 								
								 $div = $("#inquiryFormReply");
								 $div.html("");	// 해당 html를 초기화															 
								 $.each(data, function(index, value){									
									 $p1 = $("<p></p>").text(value.membernickname).append("   :   ").append(value.pititle).append("      [ ").append(value.pidate).append(" ]"); // 닉네임 : ~내용    [작성일자]
									 $p2 = $("<p></p>").text(value.picontents);
									 $p3 = $("<p style='color:#8b0000'></p>").text("└  관리자   :     ").append(value.pianswer).append("    [ ").append(value.pianswerdate).append("]"); // └ 관리자 : ~답변내용   [작성일자]
									 $answer = $("<a id='insertAnswer'></a>").text("답변달기");									
									 ...
									 $hr = $("<hr>");									 									
									 if(value.pianswer != null){	// 해당 문의에 답변이 있는 경우								     
									    	 $div.append($p1);
											 $div.append($p2);
										     $div.append($p3);
										     $div.append($hr);
		...
		// 문의 답변달기		
		$("#inquiryFormReply").on("click","#insertAnswer",function(e){
			
			var pino = $(this).prev().prev().prev().prev().html(); // 해당 문의 번호 
			var content = $(this).prev().val();  // 답변 내용
				...
				$.ajax({
					...
					success:function(){
						...
						getInqueryList();    // 답변 성공 시 문의 리스트 조회
