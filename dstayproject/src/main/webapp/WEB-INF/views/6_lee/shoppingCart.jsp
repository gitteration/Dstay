...
<script>
		...
		var totalPrice = parseInt($('#totalPrice' + id).html()); 		// 상품 가격 총합
				...
				totalPrice += pdPrice;		// 눌렀을 시 해당 상품 총금액
				totalAmountSum += 1;		// 수량도 1+
				totalPriceSum += pdPrice;   // 눌렀을 시 합계금액에서 해당 상품의 가격 + 
		...
		function minusTotal(id){
			...
			var totalPrice = parseInt($('#totalPrice' + id).html()); 	// 상품 가격 총합
				...
				if(totalAmountSum < 2){             // 만약 0개를 선택하는 하게되면
					alert("1개 이상 선택 하셔야 합니다.");   // 0 개는 곤란하니
					totalAmountSum = 2;            // 다시 1개로 바꿔주고
					totalPrice = pdPrice+pdPrice;  // 가격도 1개의 가격으로 돌려놓고
					totalPriceSum += pdPrice;      // 합계 금액도 유지되게끔
					$('#totalAmount').attr('value',totalAmountSum);
					$('#totalPrice').html(totalPrice)
					
				} else {						// 그게아니라면
					totalPrice -= pdPrice;      // 금액을 빼주고
					totalAmountSum -= 1;        // 수량도 뺴주고
					totalPriceSum -= pdPrice;   // 합계금액도 뺴주고
