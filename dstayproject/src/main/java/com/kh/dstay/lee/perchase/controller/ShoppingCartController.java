...
public class ShoppingCartController {
	...
	public String nonMemberCart(ShoppingCart sc, HttpSession session) { // 비회원 일 경우
		
		int result = 0;
		
		if(session.getAttribute("scList")!=null) { // 이 후 두번 째 상품 부터
			
			ArrayList<ShoppingCart> list=(ArrayList)session.getAttribute("scList"); // 밑에 생성된 세션을 가지고
			
			for(ShoppingCart s:list) {
				
				if(s.getPno()==sc.getPno()) { // 상품 중복 방지
					result = 1;
				}
			}
			
			if(result == 0) { // 중복되지 않으면 정상적으로 추가 
				sc.setCno((list.get(list.size()-1).getCno())+1); // 이전 상품의 장바구니 번호를 현재 추가할 상품의 장바구니 번에 +1 더하기 위함
				list.add(sc);
			}
			
			session.setAttribute("scList", list);
		
		}else { // 최초 상품을 장바구니에 담아주기 위함	
